...
class TargetLocations : Tracker {
	...
	protected void chooseLocations() {
		...
		for (uint i = 0; i < numLocs; ++i) {
			...
			for (uint j=0; j < 2; ++j) {
				string command = "<command class='set_marker' id='" + counter + "' atlas_index='" + (m_stageType == 'de' ? 8 + i : 3) + "' faction_id='" + j + "' text='" + (m_stageType == 'de' ? 'Bombsite' : 'Hostage') + (i+1) + "' position='" + position.toString() + "' color='#FFFFFF' size='1.0' show_at_screen_edge='1' />";
