...
// --------------------------------------------
class BombItemDeliveryInformer : BombTracker {
	protected SubStage@ m_listener;

	// --------------------------------------------
	BombItemDeliveryInformer(GameModeSND@ metagame, string itemKey, string itemName, int factionId, SubStage@ listener) {
	...
	protected void handleItemDropEvent(const XmlElement@ event) {
		...
		if (event.getStringAttribute("item_key") == m_itemKey &&
			...
			_log("BombItemDeliveryInformer, handle_item_drop_event, key = " + m_itemKey, 1);
			...
			if (info !is null) {
				...
				sendFactionMessageKey(m_metagame, m_factionId, "Bomb dropped in container " + container, a);
