...
class BombCarryMarker : Tracker {
	...
	// --------------------------------------------
	BombCarryMarker(GameModeSND@ metagame, string itemKey) {
		@m_metagame = @metagame;
		m_itemKey = itemKey;
	}
	...
	// --------------------------------------------
	void start() {
		_log("starting BombCarryMarker tracker", 1);
	}
	...
	protected void updateMarker(int characterId) {
		...
		if (info !is null) {
			...
			for (uint i = 0; i < factions.length(); ++i) {
				...
				string command = "<command class='set_marker' id='" + characterId + "' faction_id='" + factionId + "' position='" + position + "' show_in_game_view='0' atlas_index='3' size='1.0' />";
			}
		} else {
			removeMarker(characterId);
		}
